...
var require_parse_diff = __commonJS({
  "node_modules/parse-diff/index.js"(exports2, module2) {
    ...
    module2.exports = function(input) {
      ...
      var schemaContent = [[/^\\ No newline at end of file$/, eof], [/^-/, del], [/^\+/, add2], [/^\s+/, normal]];
...
var require_markdown_it_task_lists = __commonJS({
  "node_modules/markdown-it-task-lists/index.js"(exports2, module2) {
...
function execGit(args, cwd) {
  return new Promise((resolve, reject) => {
    (0, import_child_process.execFile)("git", args, { cwd, maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
      if (error) {
...
function parseFileStatus(statusLetter) {
...
function createGitService(workspaceRoot, commitSha) {
  return {
    async getRepoRoot() {
      const output = await execGit(["rev-parse", "--show-toplevel"], workspaceRoot);
      return output.trim();
    },
    async getDiff(filePath, mode) {
    ...
    async hasChanges(filePath, mode) {
      try {
    ...
    async getChangedMarkdownFiles(mode) {
      let args;
      switch (mode) {
        case "unstaged":
...
// src/diffParser.ts
var parseDiffLib = require_parse_diff();
function isBinaryDiff(rawDiff) {
...
function cleanPath(path4) {
  ...
  return path4.replace(/^[ab]\//, "");
}
function determineStatus(file, oldPath, newPath) {
  if (file.new) {
    return "added";
  }
  if (file.deleted) {
...
function reconstructContent(currentContent, hunks) {
  if (hunks.length === 0) {
...
function resolveImagePaths(html, basePath) {
  return html.replace(
    ...
    (_match, prefix, src, suffix) => {
      const resolved = path2.posix.join(basePath, src);
...
function createRenderer(options) {
  ...
  md.use(import_markdown_it_task_lists.default, { enabled: false });
  ...
  return {
    ...
    renderWithBase(markdown, basePath) {
      if (!markdown) {
        return "";
      }
...
function computeBlockDiffs(oldLines, newLines, hunks) {
  if (hunks.length === 0) {
...
function computeWordDiff(oldText, newText) {
  if (oldText === newText) {
    return {
      oldAnnotated: escapeHtml2(oldText),
      newAnnotated: escapeHtml2(newText)
    };
  }
  ...
  for (const [op, text2] of diffs) {
    const escaped = escapeHtml2(text2);
    if (op === import_diff_match_patch.default.DIFF_EQUAL) {
      oldAnnotated += escaped;
      newAnnotated += escaped;
    } else if (op === import_diff_match_patch.default.DIFF_DELETE) {
      oldAnnotated += `<span class="diff-removed-word">${escaped}</span>`;
    } else if (op === import_diff_match_patch.default.DIFF_INSERT) {
      newAnnotated += `<span class="diff-added-word">${escaped}</span>`;
    }
  }
  return { oldAnnotated, newAnnotated };
}
function wrapBlock(html, className) {
  return `<div class="${className}">${html}</div>`;
}
function highlightDiff(oldMarkdown, newMarkdown, hunks) {
  ...
  for (const block2 of blocks) {
    const oldBlockMd = block2.oldLines.join("\n");
    const newBlockMd = block2.newLines.join("\n");
    switch (block2.type) {
      case "unchanged": {
...
function applyWordDiffToHtml(renderedHtml, oldText, newText, side) {
  const dmp = new import_diff_match_patch.default();
  const diffs = dmp.diff_main(oldText, newText);
  dmp.diff_cleanupSemantic(diffs);
  ...
  for (const [op, text2] of diffs) {
    ...
    } else if (op === import_diff_match_patch.default.DIFF_DELETE && side === "old") {
      annotatedText += `<span class="diff-removed-word">${escaped}</span>`;
    } else if (op === import_diff_match_patch.default.DIFF_INSERT && side === "new") {
...
function replaceTextInHtml(html, annotatedText) {
...
function extractAnnotatedSegment(annotatedText, startIdx, length) {
  let plainCount = 0;
  let i = 0;
  let startPos = -1;
...
function createPanelManager(context, onMessage) {
  let panel;
  const disposables = [];
...
<body>
  ...
  const manager = {
    ...
    isOpen() {
      return panel !== void 0 && panel.visible;
    }
  };
...
// src/extension.ts
var LARGE_FILE_LINE_THRESHOLD = 5e4;
var INCREMENTAL_RENDER_LINE_THRESHOLD = 1e3;
var INCREMENTAL_BATCH_SIZE = 10;
var outputChannel;
var panelManager;
var fileWatcher;
var currentDiffMode = "unstaged";
var currentFilePath;
var currentCommitSha;
function isMarkdownFile2(filePath) {
...
async function runPipeline(filePath, mode, context, commitSha, changedSectionsOnly) {
  const workspaceFolder = vscode2.workspace.workspaceFolders?.[0];
  if (!workspaceFolder) {
    vscode2.window.showErrorMessage("No workspace folder is open.");
    return;
  }
  ...
  try {
    ...
    if (error instanceof GitError) {
      switch (error.code) {
        ...
        case "FILE_NOT_TRACKED":
          vscode2.window.showInformationMessage(error.message);
    ...
    vscode2.window.showErrorMessage(`Git error: ${msg}`);
    log(`Unexpected error: ${msg}`);
    return;
  }
  if (!rawDiff || rawDiff.trim().length === 0) {
    vscode2.window.showInformationMessage(`No changes found for ${fileName}`);
  ...
  let currentContent;
  try {
    const fileUri = vscode2.Uri.file(
      path3.isAbsolute(filePath) ? filePath : path3.join(workspaceRoot, filePath)
    );
    const fileBytes = await vscode2.workspace.fs.readFile(fileUri);
  ...
  const shouldRenderIncrementally = lineCount > INCREMENTAL_RENDER_LINE_THRESHOLD && diffResult.hunks.length > INCREMENTAL_BATCH_SIZE && !changedSectionsOnly;
  if (shouldRenderIncrementally) {
    ...
    try {
      ...
      highlighted = {
        oldHtml: `<pre>${escapeHtml3(oldContent)}</pre>`,
        newHtml: `<pre>${escapeHtml3(newContent)}</pre>`
      };
    }
...
async function runIncrementalPipeline(oldContent, newContent, diffResult, fileName, mode, commitSha) {
  if (!panelManager) {
    return;
  }
  ...
  return (message) => {
    switch (message.type) {
      case "refresh": {
        ...
        if (currentFilePath) {
          runPipeline(currentFilePath, currentDiffMode, context, currentCommitSha).catch((err) => {
            ...
            vscode2.window.showErrorMessage(`Refresh failed: ${msg}`);
          });
        }
        break;
      }
          ...
          if (newMode && (newMode === "unstaged" || newMode === "staged" || newMode === "commit")) {
            ...
            if (currentFilePath) {
              runPipeline(currentFilePath, currentDiffMode, context, currentCommitSha).catch((err) => {
                ...
                vscode2.window.showErrorMessage(`Mode switch failed: ${msg}`);
              });
            }
          }
        }
      ...
      case "requestCommitSha": {
        ...
        (async () => {
          const sha = await vscode2.window.showInputBox({
          ...
          vscode2.window.showErrorMessage(`Failed to load commit diff: ${msg}`);
        });
        break;
      }
      default:
...
function setupFileWatcher(filePath, context) {
  ...
  const absolutePath = path3.isAbsolute(filePath) ? filePath : path3.join(vscode2.workspace.workspaceFolders?.[0]?.uri.fsPath ?? "", filePath);
  const pattern = new vscode2.RelativePattern(
    path3.dirname(absolutePath),
    path3.basename(absolutePath)
  );
  const watcher = vscode2.workspace.createFileSystemWatcher(pattern);
  const refreshHandler = () => {
    if (currentFilePath && panelManager?.isOpen()) {
      log(`File changed: ${path3.basename(absolutePath)}, refreshing preview`);
      runPipeline(currentFilePath, currentDiffMode, context, currentCommitSha).catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
...
function activate(context) {
  outputChannel = vscode2.window.createOutputChannel("Markdown Diff Preview");
  context.subscriptions.push(outputChannel);
  log("Markdown Diff Preview extension activated");
  const showChangesCommand = vscode2.commands.registerCommand(
    "markdownDiffPreview.showChanges",
    async () => {
      const editor = vscode2.window.activeTextEditor;
      if (!editor) {
        vscode2.window.showInformationMessage("No active editor found. Open a markdown file first.");
        return;
      }
      const filePath = editor.document.uri.fsPath;
      if (!isMarkdownFile2(filePath)) {
        vscode2.window.showInformationMessage("This command applies only to markdown files");
        return;
      }
      currentFilePath = filePath;
      currentDiffMode = "unstaged";
      currentCommitSha = void 0;
