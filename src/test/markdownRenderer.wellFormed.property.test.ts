...
const unorderedListArb = fc
  .array(paragraphArb, { minLength: 1, maxLength: 5 })
...
const codeBlockArb = fc
  .tuple(fc.constantFrom('', 'js', 'typescript', 'python', 'bash'), paragraphArb)
  .map(([lang, code]) => `\`\`\`${lang}\n${code}\n\`\`\``);

/**
 * Arbitrary that generates a blockquote.
 */
const blockquoteArb = fc
  .array(paragraphArb, { minLength: 1, maxLength: 3 })
...
const taskListArb = fc
  .array(fc.tuple(fc.boolean(), paragraphArb), { minLength: 1, maxLength: 4 })
...
describe('Feature: markdown-diff-visualiser, Property 4: Renderer produces well-formed HTML', function () {
  ...
  it('should produce well-formed HTML for any generated markdown document', function () {
    fc.assert(
      fc.property(markdownDocArb, (markdown) => {
        ...
        expect(
          result.valid,
          `HTML is not well-formed: ${result.reason}\nMarkdown:\n${markdown}\nHTML:\n${html}`,
        ).to.be.true;
      }),
      {
        numRuns: 100,
  ...
  it('should produce well-formed HTML for single inline elements', function () {
    fc.assert(
      fc.property(inlineFormattedArb, (markdown) => {
        const html = renderer.render(markdown);
        const result = checkWellFormedHtml(html);
        expect(
          result.valid,
